...
# Updated to the provided new URL
readonly DOWNLOAD_URL="https://downloads.cursor.com/production/7801a556824585b7f2721900066bc87c4a09b743/linux/x64/Cursor-0.48.8-x86_64.AppImage"
...
# === UPDATED FUNCTION: fetch_remote_version ===
fetch_remote_version() {
  logg prompt "Looking for the latest version online..."
  # Using curl to fetch HTTP headers
  headers=$(spinner "Fetching version info from the server..." "sleep 1 && timeout \"$VERSION_CHECK_TIMEOUT\" curl -sI \"$DOWNLOAD_URL\"")
  if [[ -z "$headers" ]]; then
    logg error "Failed to fetch headers from the server.
   • Check your internet connection.
   • Increase 'VERSION_CHECK_TIMEOUT' if needed.
   • Verify that DOWNLOAD_URL is correct: $DOWNLOAD_URL."
    return 1
  fi
  logg success "Latest version details retrieved successfully."
  # Use the URL’s basename to determine the remote file name and version
  remote_name=$(basename "$DOWNLOAD_URL")
  remote_size=$(echo "$headers" | grep -i '^Content-Length:' | awk '{print $2}' | tr -d '\r' || echo "0")
  remote_version=$(extract_version "$remote_name")
  remote_md5=$(echo "$headers" | grep -i '^ETag:' | awk '{print $2}' | sed 's/"//g' || echo "unknown")
...
# === UPDATED FUNCTION: find_local_version ===
find_local_version() {
  show_log=${1:-false}
  [[ $show_log == true ]] && spinner "Searching for a local version..." "sleep 2;"
  mkdir -p "$DOWNLOAD_DIR"
  # Using -iname (case-insensitive) to match file names like "Cursor-0.48.8-x86_64.AppImage"
  local_path=$(find "$DOWNLOAD_DIR" -maxdepth 1 -type f -iname 'cursor-*.AppImage' -printf '%T@ %p\n' 2>/dev/null | sort -nr | head -n 1 | cut -d' ' -f2-)
